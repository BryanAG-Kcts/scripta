...
export const createBD = async (_req: Request, res: Response) => {
	const bd = await new ConfigModel().createBaseDatos();

	if (!bd) {
		res.status(404).json({
			sucess: false,
			message: "No se pudo crear las tablas de la Base de Datos",
		});
		return;
	}
	...
	res.status(200).json({
		sucess: true,
		message: "Todo ha salido bien, fino pa",
	});
};
...
export const putConfig = async (req: Request, res: Response) => {
	...
	try {
		const config = await new ConfigModel().updateConfig(
			id_setting,
			state,
			tone,
			verbosity,
...
export const putDictionary = async (req: Request, res: Response) => {
	...
	try {
		const response = await new ConfigModel().postDictionary(
			settingId,
			dictionary,
		);
		if (!response) {
